...
- \*_Low-Overhead Binary Protocol_: Frames are streamed as raw binary (`Uint8Array`) directly to the canvas, saving bandwidth and CPU.
...
An optional `--quality {lossless,high,balanced,low}` enables lossy _temporal
delta_: a colour cell is only re-sent once it drifts past a tolerance from what
